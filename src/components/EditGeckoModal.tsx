...
export const EditGeckoModal: React.FC<{
  gecko: Gecko | null;
  isOpen: boolean;
  onClose: () => void;
}> = ({ gecko, isOpen, onClose }) => {
  const { geckos, updateGecko } = useGeckoContext();
  ...
  useEffect(() => {
    if (gecko) {
      setName(gecko.name || '');
      setCode(gecko.code || '');
      setGender(gecko.gender || 'unsexed');
      setHatchDate(gecko.hatchDate || '');
      setStatus(gecko.status || 'breeder');
      setBreederName(gecko.breederName || '');
      setPurchasePrice(gecko.purchasePrice ? String(gecko.purchasePrice) : '');
      setFatherId(gecko.fatherId || '');
      setMotherId(gecko.motherId || '');
      setFatherName(gecko.fatherName || '');
      setMotherName(gecko.motherName || '');
      setFatherImageUrl(gecko.fatherImageUrl || '');
      setMotherImageUrl(gecko.motherImageUrl || '');
      setWeightGrams(gecko.weightGrams ? String(gecko.weightGrams) : '');
      setNotes(gecko.notes || '');
      const existingImgs = gecko.images && gecko.images.length > 0
        ? gecko.images
        : (gecko.mainImageUrl ? [gecko.mainImageUrl] : []);
      setImages(existingImgs);
      setGenetics(gecko.genetics ? { ...DEFAULT_GENETICS, ...gecko.genetics } : { ...DEFAULT_GENETICS });
    }
  }, [gecko]);
  ...
  const maleGeckos = geckos.filter(g => g.gender === 'male' && g.id !== gecko.id);

  const femaleGeckos = geckos.filter(g => g.gender === 'female' && g.id !== gecko.id);
      ...
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 720 }}>
        <div className="modal-header">
          <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#f8fafc', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Edit3 color="#10b981" size={22} /> {gecko.name} Adatlapjának Szerkesztése
          </h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <select className="form-select" value={status} onChange={e => setStatus(e.target.value as GeckoStatus)}>
                  ...
                  <option value="deceased">Elpusztult</option>
                </select>
              </div>
              <div className="form-group" style={{ margin: 0 }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', alignItems: 'flex-start' }}>
                ...
                {/* Weight */}
                <div className="form-group" style={{ margin: 0 }}>
                  <label className="form-label">Utolsó Mért Súly (g):</label>
            ...
            <div style={{
              ...
              <h4 style={{ fontSize: '0.95rem', fontWeight: 700, color: '#34d399', margin: 0, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <Dna size={16} /> Genetikai Értékek & Morph Szerkesztése
              </h4>
          ...
          <div className="modal-footer">
            <button type="submit" className="btn btn-primary">Módosítások Mentése</button>
